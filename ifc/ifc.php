...
function ifc_shortcode($atts)
{
    ...
    if (isset($_POST['ifc_submit']) && isset($_POST['ifc_answer']) && wp_verify_nonce($_POST['ifc_nonce'], 'ifc_nonce_action')) {
        ...
        echo '<div class="alert alert-success" role="alert">Thank you for your answer.</div>';
    } else {
    ?>
...
function ifc_results_shortcode($atts)
{
    ...
    ob_start();
    ?>
    <div><?php echo esc_html($question->question); ?></div>
...
function ifc_show_results($viimeisin_id = 0, $question_id)
{
    ...
    if ($answers) {
        foreach ($answers as $answer) {
            ...
            echo '<div class="col-md-4 kysely-vastaus' . $new_class . '" data-id="' . esc_attr($answer->id) . '">';
            echo '<div class="card mb-4">';
            echo '<div class="card-body">';
            echo '<p class="card-text">' . esc_html($answer->answer) . '</p>';
            echo '</div>';
            echo '<div class="card-footer text-muted">';
            echo esc_html($answer->time);
            echo '</div>';
            echo '</div>'; // .card
            echo '</div>'; // .col-md-4
        }
